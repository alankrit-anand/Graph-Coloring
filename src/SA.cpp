...
SA::SA(double initial_temperature,int n_nodes, int n_edges ,vector<vector<int>> main_graph){
...
SA::SA(double initial_temperature,int coolMode, int alpha,int n_nodes,int n_edges ,vector<vector<int>> main_graph){
...
int SA::findFirstCollision(){
    ...
    if(current_solution.getFitness() < n_edges){
        ...
        for (int i = 0; i < n_nodes && !collision; i++){
            for(int j = 0; j < main_graph[i].size() && !collision; j++){
                pos = main_graph[i][j]; 
...
void SA::NextNeighbor(){
    ...
    int color = this->getRandom(0,n_nodes);
    ...
    int temp_fit = this->CalculateFitness(temp_solution);
    int current_fit = this->CalculateFitness(current_solution);
...
void SA::MainLoop(int max_iterations, double min_temp, int &total_iterations,int min_colors){
    int correct = 0;
    bool correct_color = false;
    for(int i = 0; i < max_iterations && !correct_color;i++){
        //cout << "\n\nIt number: " << i << endl;
...
int SA::CalculateFitness(Individual& ind){
    ...
    if(ind.getFitness() != 0){
        ...
        for(int i = 0; i < n_nodes; i++){
            ...
            for(int j = 0; j < main_graph[i].size(); j++){
                int cmp_number = ind.at( main_graph[i][j] );
