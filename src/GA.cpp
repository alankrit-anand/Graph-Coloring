#include "../header/GA.h"
    ...
    GA::GA(int n_individuals, int n_genes, int n_edges, vector<vector<int>> main_graph){
    ...
    GA::GA(int n_genes, int n_edges, vector<vector<int>> main_graph){
        this->n_genes = n_genes;
        this->n_edges = n_edges;
        this->main_graph = main_graph;
    ...
    vector<Individual> GA::FindBestIndividuals( double percentage){
        ...
        priority_queue< pair<int, int>> best_ind;
        ...
        */
        ...
        for (int i = 0; i < n_individuals; ++i) {
            ...
            best_ind.push(std::pair<int, int>(fitnessOfIndividual(i), i));
        }
    ...
    vector<Individual> GA::Reproduce(vector<Individual> population, int new_ind){
        vector<Individual> new_population;
        int parent_1, parent_2;
        Individual son(n_genes);
        int n_individuals = population.size();
    ...
    vector<Individual> GA::Mutate(vector<Individual> population, int new_ind){
        vector<Individual> new_population;
        int random_idx;
    ...
    int GA::fitnessOfIndividual(int index){
        ...
        if(population[index].getFitness() != 0){
            ...
            for(int i = 0; i < n_genes; i++){
                //cout << endl <<"i=" << i << "-  ";
                for(int j = 0; j < (int) main_graph[i].size(); j++){
                    int cmp_number = population[index].at( main_graph[i][j] );
    ...
    bool GA::CorrectColor(int n_colors, double percentage){
        int avg = 0;
        bool finish = false;
        double per = n_individuals * (percentage/100);

        for(int i = 0; i < (int) population.size() && !finish; i++){
    ...
    void GA::MainLoop(int max_iterations,int &total_iterations,double p_best,double p_cross,double p_mutation, int min_colors){
